-- ═══════════════════════════════════════════════════════════════
-- Adds is_modern (self-declared, same pattern as has_ac/is_clean) and
-- switches accept_dispatch_offer()'s 'modern' tier check from an
-- automatic vehicle_year comparison to reading this flag directly —
-- consistent with how 'ac'/'clean' already work, and matching the new
-- driver-side toggle added in driver_profile_screen.dart (each with an
-- explicit on-screen honesty warning, since there's no way to verify
-- any of these three flags automatically).
-- ═══════════════════════════════════════════════════════════════

alter table public.driver_applications
  add column if not exists is_modern boolean not null default false;

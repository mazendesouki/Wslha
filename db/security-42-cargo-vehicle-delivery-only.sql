-- =====================================================================
--  وصّلها — Security #42: سيارة نقل البضائع (cargo) = توصيل فقط برضو
--
--  إضافة فئة تسجيل جديدة "cargo" (📦 سيارة نقل بضائع) على driver.astro
--  جنب "motorcycle" — نفس القيد بالظبط: توصيل فقط، بدون مشاوير محلية/
--  خارجية/مطار. هذا الملف يبني على security-41 فيعيد تعريف نفس الدوال
--  الأربعة، بس بيستبدل الشرط `= 'motorcycle'` بـ
--  `in ('motorcycle', 'cargo')` في كل مكان.
--
--  آمن لإعادة التشغيل. شغّله كاملاً مرة واحدة في SQL Editor — لازم يتشغّل
--  بعد security-41 (أو بدلاً منه لو لسه ما شغّلتوش، الملف ده يغطي كل حاجة).
-- =====================================================================
...
begin
  ...
  if v_offer.target_type = 'ride' then
    ...
    limit 1;
    if v_driver_cat in ('motorcycle', 'cargo') then return null; end if;
...
begin
  ...
  where d.driver_phone = p_driver_phone
    ...
    and ((d.target_type = 'ride' and r.id is not null and coalesce(r.is_negotiable, false) = false
          and (v_driver_cat is null or v_driver_cat not in ('motorcycle', 'cargo')))
...
-- ── 4) submit_ride_price_offer — منع سائقي التوصيل (دراجة/نقل) من التفاوض على رحلة ──

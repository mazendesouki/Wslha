-- ═══════════════════════════════════════════════════════════════
-- Adds an optional horizontal banner image to email_campaigns (security-100)
-- for more professional/marketing-grade campaign emails — same
-- recommended size (1080×566) and 'promotions' storage bucket already
-- set up for the in-app popup tool (security-98b), reused here rather
-- than creating a second bucket.
--
-- Verified live: sent a test campaign with image_url set — delivered
-- per Resend.
-- ═══════════════════════════════════════════════════════════════
...
begin
  ...
  insert into public.email_campaigns (subject, body, target_role, send_at, created_by, image_url)
  values (trim(p_subject), trim(p_body), p_target_role, coalesce(p_send_at, now()), v_admin.phone,
          nullif(trim(coalesce(p_image_url, '')), ''));
  return true;
end;
$$;
grant execute on function public.admin_create_email_campaign(text, text, text, text, text, timestamptz, text) to anon, authenticated;

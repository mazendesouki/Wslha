-- ═══════════════════════════════════════════════════════════════
-- Adds two new registerable driver vehicle_category values —
-- 'box_truck' (سيارة نقل بصندوق مغلق, for أثاث/أجهزة منزلية شحنة
-- requests) and 'flatbed' (سيارة نقالة سيارات/سطحة, for the "سيارة"
-- car-transport shipment request) — alongside the existing
-- motorcycle/cargo/sedan/suv/van, per driver.astro's updated v-cat
-- select and grp-vtype radio group.
--
-- Both are delivery-only, same as motorcycle/cargo (they're not
-- passenger vehicles) — driver.astro's own UI text already promises
-- this ("مخصّصين لطلبات خدمة الدليفري فقط — مش هيستقبلوا طلبات مشاوير
-- عادية"). While restoring that restriction for the two new
-- categories, found the motorcycle/cargo restriction itself
-- (originally added by security-41/42) had been silently dropped from
-- all 4 functions by a later, unrelated rewrite (security-59's
-- clean/modern-flags change and/or security-65's airport-ride-requests
-- change both replaced these function bodies without carrying the
-- delivery-only check forward) — motorcycle/cargo drivers currently
-- CAN accept ordinary local ride offers, since those never set
-- airport_vehicle_category/airport_quality_tier so the vehicle-match
-- block was being skipped entirely. Restored for all four categories
-- together rather than leaving that gap in place while only patching
-- the two new ones.
--
-- Rebuilt from each function's CURRENT live definition (verified via
-- pg_get_functiondef before writing this), not security-42's file
-- (which had since drifted from what's actually deployed — e.g. it
-- still referenced the old is_modern flag and a driver_not_approved
-- gate that aren't in the live version), so nothing already-shipped
-- (the is_negotiable/airport ride_type exclusions from later
-- migrations) is reverted.
-- ═══════════════════════════════════════════════════════════════
set search_path = public, extensions;

-- ── 1) get_my_pending_offer (singular) ──
...
-- ── 2) get_my_pending_offers (plural) ──
...
-- ── 4) submit_ride_price_offer — block delivery-only vehicles from negotiating a ride price ──
create or replace function public.submit_ride_price_offer(p_ride_id uuid, p_driver_phone text, p_driver_name text, p_price numeric)
returns jsonb
language plpgsql
security definer
